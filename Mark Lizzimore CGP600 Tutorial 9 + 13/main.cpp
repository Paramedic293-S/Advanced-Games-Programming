...
HRESULT Input();
//HRESULT ISKeyPressed(unsigned char DI_Keycode);
void ShutdownD3D();
...
void RenderFrame(void)
{
	...
	ReadInputStates();
	/*ISKeyPressed(DI_Keycode);*/

	if (ISKeyPressed(DIK_ESCAPE)) DestroyWindow(g_hWnd);
	/*if (ISKeyPressed(DIK_U)) camera1->(.2);*/
	if (ISKeyPressed(DIK_UP)) camera1->forward(0.01);
	if (ISKeyPressed(DIK_DOWN)) camera1->forward(-0.01);
	if (ISKeyPressed(DIK_A)) camera1->side(0.01);
	if (ISKeyPressed(DIK_D)) camera1->side(-0.01);
	if (ISKeyPressed(DIK_W)) camera1->up(0.01);
	if (ISKeyPressed(DIK_S)) camera1->up(-0.01);
	if (ISKeyPressed(DIK_LEFT)) camera1->rotate(-0.01);
	if (ISKeyPressed(DIK_RIGHT)) camera1->rotate(0.01);
	...
	world *= XMMatrixTranslation(10, 3, -15);
